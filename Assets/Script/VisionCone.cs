...
public class VisionCone : MonoBehaviour
{
    ...
    [SerializeField] private float fov = 1080f;
    ...
    private void LateUpdate()
    {
        ...
        for (int i = 0; i <= rayCount; i++)
        {
            ...
            Vector3 dir = new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad)).normalized;

            RaycastHit2D hit = Physics2D.Raycast(transform.position, dir, maxDistance, obstacleMask);
            float hitDist = hit.collider != null ? hit.distance : viewDistance;
